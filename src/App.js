...
const DEFAULT_MCP_SERVER_URL = (process.env.REACT_APP_MCP_SERVER_URL || '').trim();
const XPRA_EMBED_URL = (process.env.REACT_APP_XPRA_EMBED_URL || '').trim();
...
const extractLogMessage = (payload) => {
...
const createLogEntry = (payload, timestamp, id) => {
  const entryPayload = payload ?? {};
  const type = entryPayload && typeof entryPayload === 'object' && entryPayload.type ? entryPayload.type : 'info';
  return {
    id: id || generateRandomId(),
    type,
    message: extractLogMessage(entryPayload),
...
const logTypeStyles = {
  info: 'border border-slate-800 bg-slate-950/60 text-slate-200',
  success: 'border border-emerald-500/40 bg-emerald-500/10 text-emerald-200',
  result: 'border border-sky-500/40 bg-sky-500/10 text-sky-100',
  error: 'border border-rose-500/40 bg-rose-500/10 text-rose-200',
  cancelled: 'border border-slate-700 bg-slate-900/60 text-slate-300',
...
const MCPAgentConsole = () => {
  const [serverUrl, setServerUrl] = useState(DEFAULT_MCP_SERVER_URL);
  const [prompt, setPrompt] = useState('');
  const [isRunning, setIsRunning] = useState(false);
  const [formError, setFormError] = useState(null);
  const [tasks, setTasks] = useState({ active: [], completed: [], failed: [], cancelled: [] });
  ...
  const liveTaskIdRef = useRef(null);
  ...
  useEffect(() => {
    liveTaskIdRef.current = liveTaskId;
  }, [liveTaskId]);
  ...
  const normalizeStoredEntries = useCallback((entries = []) => {
    return entries.map((entry, index) => {
      const payload = entry && typeof entry === 'object' && 'payload' in entry ? entry.payload : entry;
      const timestamp = entry && typeof entry === 'object' && entry.timestamp ? entry.timestamp : undefined;
      return createLogEntry(payload, timestamp, `${timestamp || 'entry'}-${index}-${generateRandomId()}`);
    });
  }, []);
  ...
  const fetchTaskMetadata = useCallback(async (taskId) => {
    try {
  ...
  const loadTaskLog = useCallback(
    async (taskId) => {
      ...
      if (taskId === liveTaskIdRef.current) {
        setSelectedTaskId(taskId);
        try {
          await fetchTaskMetadata(taskId);
        } catch (error) {
          // ignore metadata errors while live streaming
        }
        setLogError(null);
      ...
      try {
        const [logResponse] = await Promise.all([
          ...
          fetchTaskMetadata(taskId),
        ]);
        if (!logResponse.ok) {
          throw new Error('Failed to load task log');
        }
        const data = await logResponse.json();
        setSelectedTaskId(taskId);
        setSelectedTaskLog(normalizeStoredEntries(data.entries || []));
        setLogError(null);
      } catch (error) {
        setLogError(error.message || 'Unable to load task log');
      } finally {
        setIsLogLoading(false);
      }
    },
    [fetchTaskMetadata, normalizeStoredEntries]
  );
  ...
  const cancelTask = useCallback(
    async (taskId) => {
      ...
      try {
        ...
        await loadTasks();
        if (taskId === liveTaskIdRef.current && abortControllerRef.current) {
          abortControllerRef.current.abort();
        }
      } catch (error) {
        setFormError(error.message || 'Unable to cancel the task');
      }
    },
    [loadTasks]
  );
    ...
    async (taskId) => {
      ...
      if (taskId === liveTaskIdRef.current) {
        try {
          ...
          // ignore metadata errors for active tasks
        }
        setLogError(null);
        return;
      }
  ...
  const startTask = async (event) => {
    ...
    if (!serverUrl.trim()) {
      setFormError('MCP server URL is required.');
      return;
    }
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/run-task`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ task: prompt, server_url: serverUrl }),
        signal: controller.signal,
      });

      if (!response.ok || !response.body) {
        throw new Error('Failed to start MCP agent task');
      }
      ...
      const processBuffer = () => {
        ...
        while (separatorIndex !== -1) {
          ...
          const payloadString = dataLines.map((line) => line.replace(/^data:\s*/, '')).join('');
          if (!payloadString) {
            continue;
          }
          ...
          try {
            ...
            if (parsed?.type === 'task' && parsed.taskId) {
              setLiveTaskId(parsed.taskId);
              liveTaskIdRef.current = parsed.taskId;
              setLiveLogs([]);
              liveLogsRef.current = [];
              setSelectedTaskId(parsed.taskId);
              setSelectedTaskLog([]);
              fetchTaskMetadata(parsed.taskId).catch(() => {});
              loadTasks().catch(() => {});
            } else {
              setLiveLogs((prev) => {
            ...
            setLiveLogs((prev) => {
              const entry = createLogEntry({ type: 'warning', message: payloadString });
              const next = [...prev, entry];
              liveLogsRef.current = next;
              return next;
            });
      ...
      while (!doneReading) {
        ...
        doneReading = processBuffer();
        if (doneReading) {
          break;
        }
      }

      if (!doneReading && buffer.length) {
        processBuffer();
      }
    } catch (error) {
      if (error.name !== 'AbortError') {
        setFormError(error.message || 'Failed to execute task');
      }
    } finally {
      setIsRunning(false);
      ...
      if (taskId) {
        setSelectedTaskId(taskId);
        setSelectedTaskLog(liveLogsRef.current);
        setLiveTaskId(null);
        setLiveLogs([]);
        try {
          await loadTasks();
          ...
          // Ignore failures when refreshing logs after completion
        }
      } else {
        setLiveTaskId(null);
        setLiveLogs([]);
  ...
  const logsToDisplay = selectedTaskId && selectedTaskId === liveTaskId ? liveLogs : selectedTaskLog;
  const selectedStatus = selectedTaskMeta?.status;
  const isSelectedActive = selectedStatus === 'running' || selectedStatus === 'pending';

  return (
    <div className="grid gap-6 lg:grid-cols-[1.8fr,1fr]">
      <div className="space-y-6">
        <form onSubmit={startTask} className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900/40 p-5">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-100">Run MCP Task</h2>
              <p className="text-xs text-slate-400">Stream output from the MCP agent via Server-Sent Events.</p>
            </div>
            <button
              type="button"
              onClick={loadTasks}
              className="inline-flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-900/60 px-3 py-1.5 text-xs text-slate-200 hover:border-sky-500/60"
            >
              <RefreshCw className="h-3.5 w-3.5" /> Refresh Tasks
            </button>
          </div>
          {formError ? <div className="rounded-xl border border-rose-500/40 bg-rose-500/10 p-3 text-sm text-rose-200">{formError}</div> : null}
          <label className="text-xs uppercase tracking-wide text-slate-400">
            MCP Server URL
            <input
              value={serverUrl}
          ...
          <div className="flex items-center justify-end gap-3">
            {liveTaskId ? (
              <button
                type="button"
                onClick={() => cancelTask(liveTaskId)}
                className="inline-flex items-center gap-2 rounded-xl border border-rose-500/50 bg-rose-500/10 px-4 py-2 text-sm text-rose-200 hover:bg-rose-500/20"
              >
                <XCircle className="h-4 w-4" /> Cancel Active Task
              </button>
            ) : null}
            <button
              type="submit"
        ...
        <div className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900/40 p-5">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-100">
                {selectedTaskId ? `Task ${selectedTaskId.slice(0, 8)}…` : 'Task Output'}
              </h2>
              <p className="text-xs text-slate-400">
                {selectedTaskMeta?.created_at ? `Created ${formatTimestamp(selectedTaskMeta.created_at)}` : 'Select a task to inspect output.'}
              </p>
            </div>
            {selectedTaskId ? (
              <div className="flex items-center gap-2">
                <span
                  className={`rounded-full px-3 py-1 text-xs capitalize ${
                    statusColors[selectedStatus] || 'bg-slate-800 text-slate-300 border border-slate-700'
                  }`}
                >
                ...
                {isSelectedActive ? (
                  <button
                    type="button"
                    onClick={() => cancelTask(selectedTaskId)}
          ...
          {logError ? <div className="rounded-xl border border-rose-500/40 bg-rose-500/10 p-3 text-sm text-rose-200">{logError}</div> : null}
          ...
          {isLogLoading ? (
            ...
            <div className="space-y-3">
              {logsToDisplay.map((entry) => (
                <div key={entry.id} className={`rounded-xl px-3 py-3 text-sm ${logTypeStyles[entry.type] || logTypeStyles.info}`}>
                  <div className="flex items-center justify-between text-xs text-slate-400">
                    <span className="font-medium text-slate-300">{logTypeLabels[entry.type] || entry.type}</span>
                    <span>{entry.timestamp ? formatTimestamp(entry.timestamp) : ''}</span>
                  </div>
                  <pre className="mt-2 whitespace-pre-wrap break-words text-slate-100">{entry.message}</pre>
                </div>
              ))}
            </div>
          ) : (
            <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-5 text-sm text-slate-400">
              {selectedTaskId ? 'No log entries recorded yet for this task.' : 'Launch a task to view the live event stream.'}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-5">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-100">Task Registry</h2>
            <button
              type="button"
              onClick={loadTasks}
              className="inline-flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-900/60 px-3 py-1.5 text-xs text-slate-200 hover:border-sky-500/60"
          ...
          {tasksError ? <div className="mt-3 rounded-xl border border-rose-500/40 bg-rose-500/10 p-3 text-sm text-rose-200">{tasksError}</div> : null}
          {isTasksLoading ? (
                    ...
                    <div className="space-y-2">
                      {tasks[key].map((task) => (
                        <button
                          ...
                          className={`w-full rounded-xl border border-slate-800 bg-slate-950/60 p-3 text-left transition hover:border-sky-500/40 ${
                            selectedTaskId === task.task_id ? 'border-sky-500/60 bg-slate-900/80' : ''
                          }`}
                        >
                          <div className="flex items-start justify-between gap-2">
                            <p className="text-sm font-medium text-slate-100 line-clamp-2">{task.prompt}</p>
                            <span
                              className={`shrink-0 rounded-full px-2.5 py-1 text-xs capitalize ${
                                statusColors[task.status] || 'bg-slate-800 text-slate-300 border border-slate-700'
                              }`}
                            >
                              {task.status}
                            </span>
                          ...
                          <div className="mt-2 flex flex-wrap items-center gap-3 text-[11px] text-slate-400">
                            <span>Created {task.created_at ? formatTimestamp(task.created_at) : 'Unknown'}</span>
                            <span>Updated {task.updated_at ? formatTimestamp(task.updated_at) : 'Unknown'}</span>
                          </div>
                        </button>
                      ))}
                    </div>
                  ) : (
                    <div className="rounded-lg border border-slate-800 bg-slate-950/60 p-3 text-xs text-slate-500">No tasks.</div>
                  )}
                </div>
              ))}
            </div>
        ...
        {XPRA_EMBED_URL ? (
          <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/40">
            <div className="flex items-center justify-between border-b border-slate-800 px-5 py-3">
              <div>
                <h2 className="text-lg font-semibold text-slate-100">XPRA Session</h2>
                <p className="text-xs text-slate-400">Interactive desktop forwarded from the automation workspace.</p>
              </div>
              <a
                href={XPRA_EMBED_URL}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-1 rounded-lg border border-slate-700 bg-slate-900/60 px-2.5 py-1 text-xs text-slate-200 hover:border-sky-500/60"
              >
                Open <ExternalLink className="h-3.5 w-3.5" />
              </a>
            </div>
            <div className="h-[420px] w-full bg-slate-950">
              <iframe
                title="XPRA Session"
                src={XPRA_EMBED_URL}
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-7xl px-6 py-10 space-y-6">
        ...
        <main className="space-y-8">
          ...
          {activeTab === 'agent' ? <MCPAgentConsole /> : null}
        </main>
